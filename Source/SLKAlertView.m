...
+ (instancetype)showWithTitle:(NSString *)title message:(NSString *)message buttonTitle1:(NSString *)title1 buttonTitle2:(NSString *)title2 buttonTitle3:(NSString *)title3 buttonHandler1:(SLKAlertViewBlock)handler1 buttonHandler2:(SLKAlertViewBlock)handler2 buttonHandler3:(SLKAlertViewBlock)handler3 {
  SLKAlertView *alert = [[SLKAlertView alloc] initWithTitle:title message:message];
  if (alert) {
    if (title1) [alert addButtonWithTitle:title1 type:SLKAlertViewButtonTypeCancel handler:handler1];
    if (title2) [alert addButtonWithTitle:title2 type:SLKAlertViewButtonTypeDefault handler:handler2];
    if (title3) [alert addButtonWithTitle:title3 type:SLKAlertViewButtonTypeDefault handler:handler3];
    [alert show];
  }
  return alert;
}
...
- (UIView *)containerView {
  if (!_containerView) {
    ...
    _containerView.alpha               = self.viewAlpha;
...
- (void)validateLayout {
  ...
  if (self.items.count > 0) {
    ...
    CGFloat t = (self.items.count == 3 ? 3 : 1);
    y = (CGRectGetMaxY(self.containerView.bounds) - BUTTON_HEIGHT * t) + BUTTON_BORDER_WIDTH * t;
    ...
    if (self.items.count == 1) {
      ...
      button = self.buttons[1];
      button.frame = CGRectMake(- BUTTON_BORDER_WIDTH, y + BUTTON_HEIGHT - BUTTON_BORDER_WIDTH, width, BUTTON_HEIGHT);

      button = self.buttons[2];
      button.frame = CGRectMake(- BUTTON_BORDER_WIDTH, y + (BUTTON_HEIGHT - BUTTON_BORDER_WIDTH) * 2, width, BUTTON_HEIGHT);
    }
  }
}
